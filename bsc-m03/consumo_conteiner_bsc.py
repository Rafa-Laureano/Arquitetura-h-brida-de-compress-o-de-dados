...
Medição robusta de CPU e memória durante:
  ...
  (2) compressão com BSC dentro de cgroup v2 -> phase=compress (cgroup)
...
# =============== CONFIG ===============
INPUT_FILE = "iot_huff.bin"
BSC_PATH   = "./bsc-m03"             # caminho do compressor (aqui: BSC)
COPIES     = 500                     # número de cópias no contêiner
WORKDIR    = Path("consumo_iot-huff_bsc")
OUTDIR     = Path("final_consumo_iot-huff_bsc")
CSV_PATH   = WORKDIR / "iot-huff_pack_bsc.csv"
SUMMARY_JSON = WORKDIR / "summary.json"
SAMPLE_DT  = 0.005                   # 50 ms (ajuste se quiser menos ruído)
# ======================================
...
def cgv2_create(name: str) -> Optional[Path]:
    root = cgv2_root()
    if not root:
        return None
...
def run_bsc_and_measure(container_path: Path, bsc_out: Path):
    # CORRETO PARA BSC: "./bsc-m03 e input output"
    cmd = [BSC_PATH, "e", str(container_path), str(bsc_out)]

    root = cgv2_root()
    if root:
        cgname = f"bsc_{int(time.time())}"
        cg = cgv2_create(cgname)
        if cg:
            # inicia o compressor
            p = subprocess.Popen(cmd, preexec_fn=os.setsid, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            try:
                ...
                out = p.communicate()[0]
                p.kill(); p.wait()
                return run_bsc_with_pidstat(container_path, bsc_out)
            ...
            if rc != 0:
                raise RuntimeError(f"BSC falhou (rc={rc})")
            return

    # Sem cgroup v2 ou não deu pra usar → fallback pidstat
    return run_bsc_with_pidstat(container_path, bsc_out)

def run_bsc_with_pidstat(container_path: Path, bsc_out: Path):
    """
    ...
    p = subprocess.Popen([BSC_PATH, "e", str(container_path), str(bsc_out)])
...
def main():
    ...
    bsc_out   = WORKDIR / f"container_{COPIES:04d}.bsc"
    ...
    t0 = time.time()
    run_bsc_and_measure(container, bsc_out)
    t_comp = time.time() - t0

    if not bsc_out.exists():
        raise RuntimeError("BSC não gerou o arquivo de saída. Verifique se o comando está correto: './bsc-m03 e input output'.")

    comp_bytes = bsc_out.stat().st_size

    # Copia artefatos finais
    fin_container = OUTDIR / f"final_container_N_{COPIES:04d}.bin"
    fin_bsc       = OUTDIR / f"final_container_N_{COPIES:04d}.bsc"
    try:
        import shutil
        shutil.copy2(container, fin_container)
        shutil.copy2(bsc_out, fin_bsc)
    except Exception:
        pass
    ...
    print("\n==== RESUMO ====")
    print(f"Contêiner: {human(cont_bytes)} | BSC: {human(comp_bytes)} | Redução: {summary['reduction_pct']:.2f}%")
    print(f"Pack: {t_pack:.3f}s | Compress: {t_comp:.3f}s")
    print(f"CSV: {CSV_PATH}")
    print(f"Resumo JSON: {SUMMARY_JSON}")
    print(f"Saídas finais: {fin_container} , {fin_bsc}")
