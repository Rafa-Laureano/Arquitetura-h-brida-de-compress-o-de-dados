...
GMIX_PATH  = "./lstm-compress"       # caminho do compressor (aqui: LSTM-Compress)
COPIES     = 500                    # número de cópias no contêiner (5 p/ teste)
WORKDIR    = Path("consumo_iot-huff_lstm")
OUTDIR     = Path("final_consumo_iot-huff_lstm")
CSV_PATH   = WORKDIR / "iot-huff_pack_lstm.csv"
...
def run_gmix_and_measure(container_path: Path, gmix_out: Path):
    # Aqui apenas trocamos o binário chamado (GMIX_PATH agora é ./lstm-compress)
    cmd = [GMIX_PATH, "-c", str(container_path), str(gmix_out)]
...
def run_gmix_with_pidstat(container_path: Path, gmix_out: Path):
    ...
    p = subprocess.Popen([GMIX_PATH, "-c", str(container_path), str(gmix_out)])
