"""
 Pipeline de RE-compressão com contêiner de metadados (versão LSTM-Compress)
...
• Re-comprime o contêiner com LSTM-Compress a cada iteração.
...
• Gera artefatos finais (contêiner e .lstm) na pasta "final" com manifesto JSON.
"""
...
# CONFIG
# =============================
INPUT_FILE = "log_huff.bin"
LSTM_PATH  = "./lstm-compress"   # caminho do binário LSTM-Compress
THRESHOLD_PCT = -5.0
MAX_ITERS = 300
WORKDIR = Path("work_lstm_log_huff")
FINALDIR = Path("final_log_huff")
CSV_LOG  = WORKDIR / "recompress_log-huff_lstm.csv"
...
def lstm_compress(inp: Path, out: Path, timeout_s: int):
    """Executa o lstm-compress e retorna métricas básicas."""
    cmd = [LSTM_PATH, "-c", str(inp), str(out)]
    t0 = time.time()
    p = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout_s)
    elapsed = time.time() - t0
    out_text = (p.stdout or "") + (p.stderr or "")
    if p.returncode != 0:
        raise RuntimeError(f"LSTM-Compress falhou (rc={p.returncode}):\n{out_text}")
    ...
    if not csv_path.exists():
        with csv_path.open('w', newline='', encoding='utf-8') as f:
            w = csv.DictWriter(f, fieldnames=[
                "timestamp","iter","copies","container_bytes","lstm_bytes",
                "reduction_pct","time_s","cross_entropy","block_types",
                "container_path","lstm_path"
            ])
            w.writeheader()
...
def main():
    ...
    for it in range(1, MAX_ITERS+1):
        ...
        lstm_out = iter_dir / f"container_{it:03d}.lstm"
        ...
        while True:
            try:
                meta = lstm_compress(container_path, lstm_out, timeout_s)
                break
        ...
        if not Path(lstm_out).exists():
            raise RuntimeError(f"LSTM-Compress não gerou o arquivo de saída esperado: {lstm_out}")
    ...
    manifest = {
        ...
        "best": {
            ...
            "lstm_bytes": best["meta"]["compressed"],
