...
# =============== CONFIG ===============
INPUT_FILE = "log_lzw.bin"
cmix_PATH  = "./cmix"                # caminho do cmix
COPIES     = 100                    # número de cópias no contêiner (5 p/ teste)
WORKDIR    = Path("consumo_log-lzw_cmix")
OUTDIR     = Path("final_consumo_log-lzw_cmix")
CSV_PATH   = WORKDIR / "log-lzw_pack_cmix.csv"
...
def run_cmix_and_measure(container_path: Path, cmix_out: Path):
    cmd = [cmix_PATH, "-c", str(container_path), str(cmix_out)]

    root = cgv2_root()
    if root:
        cgname = f"cmix_{int(time.time())}"
        ...
        if cg:
            ...
            try:
                ...
                p.kill(); p.wait()
                return run_cmix_with_pidstat(container_path, cmix_out)
            ...
            if rc != 0:
                raise RuntimeError(f"cmix falhou (rc={rc})")
            return

    # Sem cgroup v2 ou não deu pra usar → fallback pidstat
    return run_cmix_with_pidstat(container_path, cmix_out)

def run_cmix_with_pidstat(container_path: Path, cmix_out: Path):
    """
    ...
    p = subprocess.Popen([cmix_PATH, "-c", str(container_path), str(cmix_out)])
...
def main():
    ...
    cmix_out  = WORKDIR / f"container_{COPIES:04d}.cmix"
    ...
    t0 = time.time()
    run_cmix_and_measure(container, cmix_out)
    t_comp = time.time() - t0
    comp_bytes = cmix_out.stat().st_size

    # Copia artefatos finais
    fin_container = OUTDIR / f"final_container_N_{COPIES:04d}.bin"
    fin_cmix      = OUTDIR / f"final_container_N_{COPIES:04d}.cmix"
    try:
        import shutil
        shutil.copy2(container, fin_container)
        shutil.copy2(cmix_out, fin_cmix)
    except Exception:
        pass
    ...
    summary = {
        ...
        "final_cmix": str(fin_cmix.resolve()),
        "notes": {
    ...
    print("\n==== RESUMO ====")
    print(f"Contêiner: {human(cont_bytes)} | cmix: {human(comp_bytes)} | Redução: {summary['reduction_pct']:.2f}%")
    print(f"Pack: {t_pack:.3f}s | Compress: {t_comp:.3f}s")
    print(f"CSV: {CSV_PATH}")
    print(f"Resumo JSON: {SUMMARY_JSON}")
    print(f"Saídas finais: {fin_container} , {fin_cmix}")
